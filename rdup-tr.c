 ...
 */
static void stdin2archive(void)
{
	...
	if (opt_output == O_RDUP) {
		...
		if (j != ARCHIVE_OK) {
			msg(_("Failed to set archive type to %s"),
			    o_fmt[opt_output]);
			exit(EXIT_FAILURE);
		} else {
	...
	while ((rdup_getdelim(&buf, &i, delim, stdin)) != -1) {
		if (opt_output != O_RDUP) {
			tmpname =
			    g_strdup_printf("%s/%s", tmpenv, "rdup-XXXXXX");
			tmpfd = mkstemp(tmpname);
			if (tmpfd == -1) {
				msg(_
				    ("Failure to open temporary file `%s\': %s\n"),
		...
		if (opt_output != O_RDUP) {
			if (rdup_entry->f_lnk == 1) {
				...
				hlinks =
				    g_slist_append(hlinks,
						   entry_dup(rdup_entry));
				g_free(rdup_entry);
				tmpclean(opt_output, tmpfd, tmpname);
				continue;
			}
		...
		while ((bytes = block_in_header(stdin)) > 0) {
			...
			if (block_in(stdin, bytes, fbuf) == -1) {
				msg(_("Failure to read from stdin: %s"),
				    strerror(errno));
				tmpclean(opt_output, tmpfd, tmpname);
				exit(EXIT_FAILURE);
			}
			...
			if (opt_output != O_RDUP) {
				...
				if (write(tmpfd, fbuf, bytes) != bytes) {
					msg(_
					    ("Failure to write to temporary file `%s\': %s"),
					    tmpname, strerror(errno));
					tmpclean(opt_output, tmpfd, tmpname);
					exit(EXIT_FAILURE);
				}
		...
		if (opt_output != O_RDUP) {
			...
			if (lseek(tmpfd, 0, SEEK_SET) == -1) {
				msg(_
				    ("Failure to rewind temporary file `%s\': %s"),
				    tmpname, strerror(errno));
				tmpclean(opt_output, tmpfd, tmpname);
				exit(EXIT_FAILURE);
			}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "cP:O:t:LhVvX:Y:")) != -1) {
		...
		case 'X':
			...
			if (opt_decrypt_key) {
				msg(_
				    ("Will not do both encryption and decryption"));
				exit(EXIT_FAILURE);
			}
		...
		case 'Y':
			...
			if (opt_crypt_key) {
				msg(_
				    ("Can not do both encryption and decryption"));
				exit(EXIT_FAILURE);
			}
