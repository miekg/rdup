...
gint
rdup_write_data( __attribute__ ((unused)) struct rdup * e, char *buf,
		size_t len)
{
...
struct stat *stat_from_rdup(struct rdup *e)
{
	struct stat *s;
...
static void strtime(time_t time, gchar * str)
{
	struct tm *t;
	t = localtime(&time);
...
static void rdup_strmode(mode_t mode, gchar * str)
{
	str[0] = mode & S_IRUSR ? 'r' : '-';
	str[1] = mode & S_IWUSR ? 'w' : '-';
	str[2] = (mode & S_ISUID ? (mode & S_IXUSR ? 's' : 'S')
		  : (mode & S_IXUSR ? 'x' : '-'));
	str[3] = mode & S_IRGRP ? 'r' : '-';
	str[4] = mode & S_IWGRP ? 'w' : '-';
	str[5] = (mode & S_ISGID ? (mode & S_IXGRP ? 's' : 'S')
		  : (mode & S_IXGRP ? 'x' : '-'));
	str[6] = mode & S_IROTH ? 'r' : '-';
	str[7] = mode & S_IWOTH ? 'w' : '-';
	str[8] = (mode & S_ISVTX ? (mode & S_IXOTH ? 't' : 'T')
		  : (mode & S_IXOTH ? 'x' : '-'));
	str[9] = ' ';
	str[10] = '\0';
}
...
gint rdup_write_table(struct rdup *e, FILE * f)
{
	...
	if ((S_ISLNK(e->f_mode) || e->f_lnk == 1)) {
		...
		fprintf(f, "% 9ld ",
			(unsigned long)(e->f_name_size - e->f_size - 4));
